...
def printPerformance(model, dataset_FD:StructTorchArray, dataset_pred:StructTorchArray, threshold_const_eval=0.001):
    ...
    with torch.no_grad():
        ...
        constraintError = (
            torch.einsum('ncj,nj->nc',dataset_FD.A, dataset_FD.ddq) - dataset_FD.b
        )
        # evaluate only on test data that fulfills the contraint equation A*ddq=b 
        idxValid = (torch.abs(constraintError) < threshold_const_eval).flatten()
        # select samples in the dataset that are valid
        dataset_pred = dataset_pred[idxValid]
        dataset_FD   = dataset_FD[idxValid]
        
        # reduce dataset size to fit in memory
        dataset_pred = dataset_pred[0:1000]
        dataset_FD   = dataset_FD[0:1000]

        s = f'Performance evaluated on {len(dataset_pred)} test points: \n'
        # evaluate on training data
        qn_pred  = model.qn(dataset_pred)
        dqn_pred = model.dqn(dataset_pred)
        ddq_FP   = model.ddq(dataset_FD)
        ddq_base = torch.einsum('nab,nb->na', dataset_FD.L, dataset_FD.b) + torch.einsum('nab,nbc,nc->na', dataset_FD.T, dataset_FD.Minv, dataset_FD.f + dataset_FD.g + dataset_FD.tau )

        KPI.RMSE = {
            'ddq':    torch.sqrt(torch.mean((ddq_FP.mean   - dataset_FD.ddq)**2,   dim=0)).cpu(),
            'dqn':    torch.sqrt(torch.mean((dqn_pred.mean - dataset_pred.dqn)**2, dim=0)).cpu(),
            'qn':     torch.sqrt(torch.mean((qn_pred.mean  - dataset_pred.qn)**2,  dim=0)).cpu()
        }
        s += (
            f'Prediction performance: (RMSE)\n'
            f'   ddq:    {RMSE["ddq"]}\n'
            f'   ddq_Fa: {torch.sqrt(torch.mean((ddq_base - dataset_FD.ddq)**2,   dim=0)).cpu()}\n'
            f'   dqn:    {RMSE["dqn"]}\n'
            f'   qn:     {RMSE["qn"]}\n'
        )

        KPI.MAE = {
            'ddq':    torch.mean(torch.abs(ddq_FP.mean   - dataset_FD.ddq),   dim=0).cpu(),
            'dqn':    torch.mean(torch.abs(dqn_pred.mean - dataset_pred.dqn), dim=0).cpu(),
            'qn':     torch.mean(torch.abs(qn_pred.mean  - dataset_pred.qn),  dim=0).cpu()
        }
        s += (
            f'Prediction performance: (MAE)\n'
            f'   ddq:    {MAE["ddq"]}\n'
            f'   ddq_Fa: {torch.mean(torch.abs(ddq_base - dataset_FD.ddq),   dim=0).cpu()}\n'
            f'   dqn:    {MAE["dqn"]}\n'
            f'   qn:     {MAE["qn"]}\n'
        )

        ddq_pctInsideConfidenceRegion = torch.logical_and(
            dataset_FD.ddq >= ddq_FP.confidence_region()[0],
            dataset_FD.ddq <= ddq_FP.confidence_region()[1]
        )
        dqn_pctInsideConfidenceRegion = torch.logical_and(
            dataset_pred.dqn >= dqn_pred.confidence_region()[0],
            dataset_pred.dqn <= dqn_pred.confidence_region()[1]
        )
        qn_pctInsideConfidenceRegion = torch.logical_and(
            dataset_pred.qn >= qn_pred.confidence_region()[0],
            dataset_pred.qn <= qn_pred.confidence_region()[1]
        )
        KPI.ConfReg = {
            'ddq': torch.mean( ddq_pctInsideConfidenceRegion.float(), dim=0).cpu(),
            'dqn': torch.mean( dqn_pctInsideConfidenceRegion.float(), dim=0).cpu(),
            'qn':  torch.mean( qn_pctInsideConfidenceRegion.float(), dim=0).cpu()
        }
        s += (
            f'Uncertainty prediction performance: (% or predictions inside confidence region)\n'
            f'   ddq: {ConfReg["ddq"]}\n'     
            f'   ddq: {ConfReg["dqn"]}\n'  
            f'   ddq: {ConfReg["qn"]}\n'
        )
        ...
        pred_ConstraintError = (
            torch.einsum('ncj,nj->nc',dataset_FD.A, ddq_FP.mean) - dataset_FD.b
        ).cpu().numpy()
        KPI.ConstError = {
            'mean': np.mean(np.abs(pred_ConstraintError)),
            'min':  np.min(np.abs(pred_ConstraintError)),
        ...
        s += (
            f'Constraint satisfaction performance: abs(A @ ddq - b)\n'
            f'   Mean: {ConstError["mean"]:.3e}\n'
            f'   Min:  {ConstError["min"]:.3e}\n'
            f'   Max:  {ConstError["max"]:.3e}\n'
        )
...
def evalPrediction(
    model, nq:int, 
    dataset_FD_test:StructTorchArray, dataset_pred_test:StructTorchArray, 
    # dataset_FD_train:StructTorchArray=None, dataset_pred_pred:StructTorchArray=None,
    timeAsXAxis=False, xlim=None, datasetMaxSize=1000
    ):

    dataset_FD   = dataset_FD_test[:datasetMaxSize]
    dataset_pred = dataset_pred_test[:datasetMaxSize]
    ...
    with torch.no_grad():
        ...
        fig, axs = plt.subplots(3, nq, figsize=(15,8))
        for j in range(nq):
            
            axs[0,j].grid(True)
            axs[0,j].set_title(f'$\mu_{{qn,{j}}}$')
            axs[0,j].set_xlabel(f'time [s]' if timeAsXAxis else f'time steps')
            axs[0,j].plot( 
                dataset_pred.t.cpu() if timeAsXAxis else range(len(dataset_pred.t)), 
                dataset_pred.qn[:,j].cpu(), 
                marker='', ls='--', lw=1, color='k', mfc=None, zorder=3
            )
            axs[0,j].plot( 
                dataset_pred.t.cpu() if timeAsXAxis else range(len(dataset_pred.t)), 
            ...
            axs[0,j].fill_between( 
                dataset_pred.t.flatten().cpu() if timeAsXAxis else range(len(dataset_pred.t)),
                qn_pred.confidence_region()[0][:,j].cpu(),
                qn_pred.confidence_region()[1][:,j].cpu(),
                color='purple', alpha=0.3, zorder=1
            )
            
            axs[1,j].grid(True)
            axs[1,j].set_title(f'$\mu_{{dqn,{j}}}$')
            axs[1,j].set_xlabel(f'time [s]' if timeAsXAxis else f'time steps')
            axs[1,j].plot( 
                dataset_pred.t.cpu() if timeAsXAxis else range(len(dataset_pred.t)), 
                dataset_pred.dqn[:,j].cpu(), 
                marker='', ls='--', lw=1, color='k', mfc=None, zorder=3
            )
            axs[1,j].plot( 
                dataset_pred.t.cpu() if timeAsXAxis else range(len(dataset_pred.t)), 
            ...
            axs[1,j].fill_between( 
                dataset_pred.t.flatten().cpu() if timeAsXAxis else range(len(dataset_pred.t)),
                dqn_pred.confidence_region()[0][:,j].cpu(),
                dqn_pred.confidence_region()[1][:,j].cpu(),
                color='purple', alpha=0.3, zorder=1
            )

            axs[2,j].grid(True)
            axs[2,j].set_title(f'$\mu_{{ddq,{j}}}$')
            axs[2,j].set_xlabel(f'time [s]' if timeAsXAxis else f'time steps')
            axs[2,j].plot( 
                dataset_FD.t.cpu()  if timeAsXAxis else range(len(dataset_pred.t)), 
                dataset_FD.ddq[:,j].cpu(), 
                marker='', ls='--', lw=1, color='k', mfc=None, zorder=3
            )
            axs[2,j].plot( 
                dataset_FD.t.cpu()  if timeAsXAxis else range(len(dataset_pred.t)), 
            ...
            axs[2,j].fill_between( 
                dataset_FD.t.flatten().cpu() if timeAsXAxis else range(len(dataset_pred.t)),
                ddq_FP.confidence_region()[0][:,j].cpu(),
                ddq_FP.confidence_region()[1][:,j].cpu(),
                color='purple', alpha=0.3, zorder=1
            )
...
def evalPredictionError(
    model, nq:int, 
    dataset_FD:StructTorchArray, dataset_pred:StructTorchArray, 
    timeAsXAxis:bool=False, datasetMaxSize=1000
    ):

    dataset_FD   = dataset_FD[0:datasetMaxSize]
    dataset_pred = dataset_pred[0:datasetMaxSize]
    ...
    with torch.no_grad():
        ...
        plotStyle = {'marker':'', 'ls':'-', 'lw':1, 'color':'purple', 'mfc':None}

        fig, axs = plt.subplots(3, nq, figsize=(15,8))
        for j in range(nq):

            axs[0,j].grid(True)
            axs[0,j].set_title(f'$q{j}_{{k+1, pred}} - q{j}_{{k+1, true}}$')
            axs[0,j].set_xlabel(
                f'time [s]' if timeAsXAxis else f'dataset'
            )
            axs[0,j].plot(
                dataset_pred.t.cpu() if timeAsXAxis else range(len(dataset_pred.t)), 
                qn_pred.mean[:,j].cpu() *0, 
                lw=1, color='black'
            )
            axs[0,j].plot(
                dataset_pred.t.cpu() if timeAsXAxis else range(len(dataset_pred.t)), 
                qn_pred.mean[:,j].cpu() - dataset_pred.qn[:,j].cpu(), 
                **plotStyle
            )
            axs[0,j].fill_between( 
                dataset_pred.t.flatten().cpu() if timeAsXAxis else range(len(dataset_pred.t)),
                qn_pred.confidence_region()[0][:,j].cpu() - dataset_pred.qn[:,j].cpu(),
                qn_pred.confidence_region()[1][:,j].cpu() - dataset_pred.qn[:,j].cpu(),
                color='purple', alpha=0.3
            )

            axs[1,j].grid(True)
            axs[1,j].set_title(f'$\dot q{j}_{{k+1, pred}} - \dot q{j}_{{k+1, true}}$')
            axs[1,j].set_xlabel(
                f'time [s]' if timeAsXAxis else f'dataset'
            )
            axs[1,j].plot(
                dataset_pred.t.cpu()  if timeAsXAxis else range(len(dataset_pred.t)), 
                dqn_pred.mean[:,j].cpu() *0, 
                lw=1, color='black'
            )
            axs[1,j].plot(
                dataset_pred.t.cpu()  if timeAsXAxis else range(len(dataset_pred.t)), 
                dqn_pred.mean[:,j].cpu() - dataset_pred.dqn[:,j].cpu(), 
                **plotStyle
            )
            axs[1,j].fill_between( 
                dataset_pred.t.flatten().cpu() if timeAsXAxis else range(len(dataset_pred.t)),
                dqn_pred.confidence_region()[0][:,j].cpu() - dataset_pred.dqn[:,j].cpu(),
                dqn_pred.confidence_region()[1][:,j].cpu() - dataset_pred.dqn[:,j].cpu(),
                color='purple', alpha=0.3
            )

            axs[2,j].grid(True)
            axs[2,j].set_title(f'$\ddot q{j}_{{k, pred}} - \ddot q{j}_{{k, true}}$')
            axs[2,j].set_xlabel(
                f'time [s]' if timeAsXAxis else f'dataset'
            )
            axs[2,j].plot(
            ...
            axs[2,j].plot(
                dataset_FD.t.cpu()  if timeAsXAxis else range(len(dataset_pred.t)), 
                ddq_FP.mean[:,j].cpu() - dataset_FD.ddq[:,j].cpu(), 
                **plotStyle
            )
            axs[2,j].fill_between( 
                dataset_FD.t.flatten().cpu() if timeAsXAxis else range(len(dataset_pred.t)),
                ddq_FP.confidence_region()[0][:,j].cpu() - dataset_FD.ddq[:,j].cpu(),
                ddq_FP.confidence_region()[1][:,j].cpu() - dataset_FD.ddq[:,j].cpu(),
                color='purple', alpha=0.3
            )
