# TODO: Look at TestFile_02 and change it to work with PyTorch
...
# from classes.torch_utils import rotZ
...
# j_chassis_lowerA = RotationalJoint(chassis,lowerA,  A_PDp=eye(3), A_SDs=eye(3), P_r_PDp=array([0,0,0]), S_r_SDs= array([-lowerA.length/2,0,0]))
# j_lowerA_upright = RotationalJoint(lowerA,upright,  A_PDp=rotZ(pi/2), A_SDs=eye(3), P_r_PDp=array([lowerA.length/2,0,0]), S_r_SDs= array([-0.1,0,0]))
# j_upright_upperA = RotationalJoint(upright, upperA, A_PDp=rotZ(pi/2), A_SDs=eye(3), P_r_PDp=array([0.1,0,0]), S_r_SDs= array([-upperA.length/2,0,0]))
# c_upperA_chassis = PositionBilateralConstraint(upperA, chassis, P_r_PDp=array([upperA.length/2,0,0]), S_r_SDs=array([0,0.2,0]))

# j_chassis_rocker = RotationalJoint(chassis, rocker, A_PDp=eye(3), A_SDs=eye(3), P_r_PDp=array([-0.1,0.3,0]), S_r_SDs= array([0,-0.05,0]))
# j_rocker_pushbar = RotationalJoint(rocker, pushbar, A_PDp=eye(3), A_SDs=eye(3), P_r_PDp=array([0.05,0,0]), S_r_SDs= array([-pushbar.length/2,0,0]))
# sd_chassis_rocker = SpringDamper(chassis, rocker, P_r_PDp=array([-0.25,0.35,0]), S_r_SDs=array([-0.05,0,0]), K=500, D=200, d0=0, radius=0.02, coils=30)

# c_pushbar_lowerA = PositionBilateralConstraint(pushbar,lowerA, P_r_PDp=array([pushbar.length/2,0,0]), S_r_SDs=array([1/2.5*lowerA.length,0,0]))
...
# suspension.setJointStates( q = array([0, 0, 0, 0, -50]) * pi/180 )
# suspension.forwardKinematics()
...
# def odefun(t,y):
#     q, qDot = y[0:nq], y[nq:]
#     qDDot, _ = suspension.forwardDynamics ( q=q, qDot=qDot )
#     return concatenate((qDot,qDDot))


# # initial conditions
# q0, dq0, ddq0  = suspension.getJointStates()
...
#         suspension.setJointStates( q=y[0:nq], qDot=y[nq:] )
#         suspension.forwardKinematics()
